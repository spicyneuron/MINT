...
We conduct comprehensive downstream evaluation to verify that MINT's perplexity improvements translate to preserved task performance. We evaluate on three standard benchmarks---ARC-Challenge (25-shot, normalized accuracy), Winogrande (5-shot), and HellaSwag (10-shot)---using \texttt{lm-evaluation-harness}~\cite{eval-harness} via the MLX backend. Evaluations cover three model families (Qwen3-8B, Qwen3-30B-A3B, Mixtral-8x7B) across six configurations each, spanning the full 3-bit to 8-bit range: min-safe (all 3-bit), MINT at the default budget, uniform 4-bit at matched size, MINT at $+30\%$ and $+60\%$ budgets, and uniform 8-bit as reference. Full results with standard errors for all configurations are reported in Appendix~\ref{app:downstream_full}.
...
The key finding across all three model families is that \textbf{MINT preserves downstream task accuracy within the expected statistical error bounds}. Specifically:

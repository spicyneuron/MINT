...
\title{MINT: Compute-Optimal Data-Free Mixed-Precision Quantization\\for Large Language Models via Rate-Distortion Optimization}
...
\begin{equation}
    \min_{\{(b_i, g_i)\}} \sum_{i=1}^{T} \pi_i \cdot \alpha_i \cdot \text{NRMSE}_i(b_i, g_i) \qquad
    \text{s.t.} \quad \sum_{i=1}^{T} \text{size}_i(b_i, g_i) \leq B
    \label{eq:objective}
\end{equation}
where $b_i$ and $g_i$ are the bit-width and group size for tensor $i$, $B$ is the user's memory budget, $\pi_i$ is a soft protection prior, and $\alpha_i$ is a learned importance weight. The key insight is that \emph{both bit-width and group size} are allocation variables---prior work optimizes bit-width alone, but our evidence shows group-size selection often provides larger quality improvements than bit-width changes.

Our contributions are:
\begin{itemize}
    \item A \textbf{compute-optimal formulation} (MCKP) that jointly optimizes per-tensor bit-width and group size, replacing ad-hoc thresholds with a principled allocation under an explicit memory constraint.
    \item \textbf{Budget-targeted deployment}: users specify an exact memory target and receive the optimal allocation. A fitted PPL prediction curve enables quality estimation \emph{before} running the pipeline, enabling hardware-specific deployment planning (e.g., iPhone 16\,GB, RTX 4090 24\,GB, Mac M4 Pro 48\,GB).
    \item \textbf{Quantizer-aligned sensitivity features}: per-group kurtosis matching quantization granularity, norm-guided probes using the model's own LayerNorm geometry, and multi-point rate-distortion curves that break circularity.
    ...
    \item A recommendation to report \textbf{median perplexity} alongside means, with evidence that mean PPL obscures the true quality ordering on models with outlier sequences.
    ...
    \item \textbf{Expert-grouped allocation} for MoE models: a constraint-aware grouping strategy that treats all experts within a SwitchLinear module as a single allocation unit with worst-case error, correctly modeling the hardware constraint that co-located experts share quantization parameters.
...
MINT operates in two passes over the model's weight tensors, followed by a compute-optimal allocation. The entire pipeline is \emph{data-free}---it requires only the pretrained weight tensors.

\subsection{Pass 1: Feature Extraction and Rate-Distortion Curves}

For each weight tensor $\mathbf{W} \in \mathbb{R}^{m \times n}$, MINT computes three categories of features plus a complete rate-distortion curve.
...
\end{equation}
Unlike spectral concentration (used in prior work), stable rank is unambiguously interpretable: lower values indicate that fewer directions carry most information, making quantization more likely to corrupt critical components.

\paragraph{Spectral tail mass.} The fraction of energy outside the top $k/10$ singular values:
\begin{equation}
    \tau(\mathbf{W}) = 1 - \frac{\sum_{i=1}^{\lfloor r/10 \rfloor} \sigma_i^2}{\sum_{i=1}^{r} \sigma_i^2}
\end{equation}

\paragraph{Log condition number.} A measure of numerical sensitivity, capped at $10.0$:
\begin{equation}
    \kappa(\mathbf{W}) = \min\!\left(10, \, \log_{10}\!\left(\frac{\sigma_1}{\sigma_{\min} + \varepsilon}\right)\right)
\end{equation}

These three features replace the single ``SVD concentration'' metric of prior data-free methods, which was criticized for having an ambiguous relationship with quantization sensitivity~\cite{bondarenko2023quantizable}. Rather than collapsing spectral information into a single score, these three features allow the allocator to distinguish tensors with similar overall energy but different spectral decay profiles.

\subsubsection{Per-Group Kurtosis Features}

Modern PTQ uses group-wise quantization (groups of $g$ weights sharing a scale factor). Global kurtosis---computed over the flattened tensor---misaligns with this granularity: a handful of outliers spike global kurtosis even though group-wise quantization isolates those outliers into local scale factors.
...
\begin{align}
    ...
    f_{\text{ratio}} &= P_{99} / P_{50} & \text{(tail heaviness variation)} \\
    f_{\text{outlier}} &= \frac{1}{K} \sum_{j=1}^{K} \mathbf{1}[\exists\, w_{j,i} : |w_{j,i} - \bar{w}_j| > 3s_j] & \text{(outlier group fraction)} \\
    f_{\text{maxmed}} &= \max(\kappa_j) / \text{median}(\kappa_j) & \text{(max-to-median ratio)}
\end{align}

This alignment between the statistical metric and the quantization mechanism directly addresses the critique that global kurtosis operates at the wrong granularity~\cite{zhao2025kurtail}.

\subsubsection{Norm-Guided Output Noise Amplification}

Estimating output sensitivity without calibration data requires probe vectors. Prior data-free methods use isotropic random probes $\mathbf{x} \sim \mathcal{U}(-1, 1)^n$, but LLM activations are highly anisotropic---massive persistent channel-wise outliers dominate inference geometry~\cite{xiao2023smoothquant,lin2024awq}.

MINT preserves the data-free property while injecting the model's own activation geometry. Each linear layer in a transformer is preceded by a normalization layer (LayerNorm or RMSNorm) with a learned scale parameter $\boldsymbol{\gamma} \in \mathbb{R}^n$. We sample probes from:
\begin{equation}
    \mathbf{x}_j \sim \mathcal{N}(\mathbf{0}, \, \text{diag}(\boldsymbol{\gamma}^2))
    \label{eq:norm_probes}
\end{equation}
This encodes the model's learned channel importance: channels with large $\gamma_c$ produce large activation magnitudes, and our probes respect this structure.

Furthermore, we replace the synthetic uniform perturbation of prior work with the \emph{actual} round-to-nearest (RTN) quantization residual:
\begin{equation}
    \Delta \mathbf{W} = Q(\mathbf{W}; 4, 128) - \mathbf{W}
\end{equation}
where $Q(\cdot; b, g)$ denotes group-wise RTN quantization at $b$ bits with group size $g$. The output noise amplification is then:
\begin{equation}
    f_{\text{out}} = \frac{\|\Delta \mathbf{W} \cdot \mathbf{X}\|_F}{\|\mathbf{W} \cdot \mathbf{X}\|_F + \varepsilon}
    \label{eq:output_noise}
\end{equation}
averaged over $p = 32$ normalized probe vectors. If a preceding normalization layer is not available, we fall back to isotropic probes $\mathbf{x} \sim \mathcal{N}(\mathbf{0}, \mathbf{I})$.

\subsubsection{Rate-Distortion Curves}
...
MINT computes a complete \textbf{rate-distortion curve}: the normalized RMSE at every valid (bit-width, group-size) configuration:
\begin{equation}
    \text{NRMSE}_i(b, g) = \frac{\text{RMS}(Q(\mathbf{W}_i; b, g) - \mathbf{W}_i)}{\text{RMS}(\mathbf{W}_i)}
\end{equation}
evaluated at configurations $\mathcal{C} = \{(2,32), (3,64), (4,32), (4,64), (4,128), (8,64), (8,128), (16,0)\}$.

This curve serves two purposes: it provides the \emph{loss function} for the allocator (the NRMSE at the chosen configuration), and it yields four derived features that characterize the \emph{shape} of each tensor's error response:
\begin{itemize}
    \item $f_{\text{auc}}$: trapezoidal area under the RD curve (overall compressibility)
    \item $f_{\text{m48}}$: $\text{NRMSE}(4,128) - \text{NRMSE}(8,128)$ (marginal gain from 4 to 8 bits)
    \item $f_{\text{m24}}$: $\text{NRMSE}(2,32) - \text{NRMSE}(4,128)$ (marginal gain from 2 to 4 bits)
    \item $f_{\text{slope}}$: ratio of low-bit to high-bit marginal slopes (curvature)
\end{itemize}

\subsubsection{SQNR Safety Veto}
...
\subsection{Pass 2: Normalization, Priors, and Allocation}

\subsubsection{eCDF Normalization}

Prior data-free methods normalize features using hardcoded linear bounds (e.g., $\text{clip}((\kappa - 0) / 10, 0, 1)$). These bounds are architecture-specific and saturate on large models---the reconstruction error metric with bounds $[0.005, 0.05]$ saturates at 1.0 for nearly all tensors on models larger than 100B parameters.

MINT replaces all hardcoded bounds with \textbf{empirical CDF (eCDF) percentile ranking}. During Pass~1, raw feature values are collected across all tensors. Each tensor's normalized score for feature $f$ is simply its rank:
\begin{equation}
    \hat{f}_i = \frac{|\{j : f_j \leq f_i\}|}{T}
\end{equation}
where $T$ is the total number of tensors. eCDF normalization is scale-invariant, never saturates, requires no architecture-specific tuning, and works identically on models from 8B to 400B+ parameters. The mapping is implemented via \texttt{np.searchsorted} on sorted feature arrays for $O(\log T)$ lookups.

\subsubsection{Soft Protection Priors}
...
\subsubsection{Budget-Constrained Allocation}

The allocation problem is a \textbf{Multiple-Choice Knapsack Problem (MCKP)}. Each tensor $i$ has a candidate set $\mathcal{C}_i$ (configurations surviving the SQNR veto). For each candidate $(b, g) \in \mathcal{C}_i$, the \emph{cost} is $\text{size}_i(b, g)$ and the \emph{loss} is $\pi_i \cdot \alpha_i \cdot \text{NRMSE}_i(b, g)$.

\begin{equation}
    \text{size}_i(b, g) = \left\lceil \frac{n_i \cdot b}{8} \right\rceil + \left\lfloor \frac{n_i}{g} \right\rfloor \cdot 4 \;\text{bytes}
    \label{eq:size}
\end{equation}
where $n_i$ is the parameter count and the second term accounts for 16-bit scale and zero-point per group.
...
\begin{enumerate}
    \item \textbf{Greedy} (default): Initialize all tensors at their lowest-cost valid configuration. Compute \emph{upgrade options}---for each tensor and each higher configuration, compute the efficiency ratio $\eta = \Delta\text{loss} / \Delta\text{size}$. Sort upgrades by $\eta$ descending and greedily apply them until the budget is exhausted. Runtime: $O(T \cdot |\mathcal{C}| \cdot \log(T \cdot |\mathcal{C}|))$, under 10\,ms for 18{,}867 tensors.
    \item \textbf{LP relaxation}: Continuous relaxation solved via \texttt{scipy.optimize.linprog} with HiGHS, followed by rounding. Near-optimal guarantees.
...
\STATE \textbf{Pass 1: Feature extraction}
\FOR{each shard file in model}
    \FOR{each 2D tensor $\mathbf{W}_i$ with $n_i \geq 1024$}
        \STATE Extract preceding LayerNorm $\boldsymbol{\gamma}$ (if available)
        \STATE Compute spectral features: $r_s, \tau, \kappa$
        \STATE Compute per-group kurtosis features: $f_{\text{kurt90}}, f_{\text{ratio}}, f_{\text{outlier}}, f_{\text{maxmed}}$
        \STATE Compute output noise amplification $f_{\text{out}}$ via Eq.~\ref{eq:output_noise}
        \STATE Compute RD curve: $\text{NRMSE}_i(b,g)$ for all $(b,g) \in \mathcal{C}$
        \STATE Compute SQNR map: $\text{SQNR}_i(b,g)$ for all $(b,g) \in \mathcal{C}$
        \STATE Collect raw features for eCDF construction
    \ENDFOR
    \STATE Free shard memory
\ENDFOR
\STATE \textbf{Pass 2: Normalize, allocate}
\STATE Fit eCDF on collected features; transform all features to percentile ranks
...
For MoE models with expert tensors stored as 3D arrays $[E, d_{\text{in}}, d_{\text{out}}]$:
\begin{itemize}
    \item \textbf{Individual analysis} ($E \leq 32$): Analyze each expert slice separately. Use worst-case features as the representative and record inter-expert variance.
    \item \textbf{Clustered analysis} ($E > 32$): Compute lightweight statistics (Frobenius norm, global kurtosis) per expert. Cluster into $\min(8, E/4)$ groups via $k$-means. Sample one expert per cluster; use worst-case across samples.
\end{itemize}
...
\begin{itemize}
    \item \textbf{NRMSE}: worst-case (maximum) across experts: $\text{NRMSE}_G(b,g) = \max_e \text{NRMSE}^{(e)}(b,g)$
    \item \textbf{SQNR}: worst-case (minimum) across experts: $\text{SQNR}_G(b,g) = \min_e \text{SQNR}^{(e)}(b,g)$
    \item \textbf{Size}: sum across all experts: $\text{size}_G(b,g) = \sum_e \text{size}^{(e)}(b,g)$
\end{itemize}
This reduces the number of allocation variables from $T \cdot E$ to $T$ (e.g., 5{,}376 expert tensors $\rightarrow$ 84 groups on Qwen2-57B-A14B), while ensuring that the solver's output is directly implementable by the runtime. The worst-case NRMSE conservatively prevents any expert from receiving an unsafely aggressive quantization.
...
\begin{table}[h]
\centering
\caption{Perplexity comparison at matched and varied model sizes. All evaluations use WikiText-2 (test, seq\_len=2048, 128 sequences, seed=42). Both mean and median PPL are reported; we recommend median as the primary metric (\S\ref{sec:mean_vs_median}). $\Delta$ columns are relative to BF16. $\dagger$Values from~\cite{xu2025qwen3quant}. $\ddagger$BF16 model (203\,GB) exceeds available memory; $\Delta$ computed vs uniform 4-bit.}
...
\midrule
\multirow{6}{*}{\shortstack[l]{Qwen3-30B\\-A3B (MoE)}}
    & BF16 baseline & 56.87 & 8.728 & --- & --- \\
    & Uniform 4-bit & 15.11 & 9.629 & --- & +10.3\% \\
...
\midrule
\multirow{4}{*}{\shortstack[l]{GLM-4.7\\-Flash}}
    & BF16 baseline & 58.16 & 11.344 & 8.706 & --- \\
    & Uniform 4-bit & 14.82 & --- & ${\sim}$11.46 & +31.6\% \\
    & v1 (best) & 15.92 & 9.930 & 9.084 & +4.3\% \\
    & \textbf{MINT} & \textbf{15.82} & \textbf{9.427} & \textbf{9.210} & \textbf{+5.8\%} \\
\midrule
\multirow{8}{*}{\shortstack[l]{Llama-4\\-Scout$^\ddagger$\\(109B MoE)}}
    & BF16 baseline & ${\sim}$203 & \multicolumn{2}{c}{\emph{exceeds 192\,GB}} & --- \\
    & MINT (no safety) & 34.62 & 23.577 & 23.714 & +198\% \\
...
--- & 56.87 GB & 8.728 & --- & \emph{BF16 (unquantized)} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Diminishing returns.} Beyond ${\sim}$19\,GB, additional budget yields marginal returns: the 19 $\rightarrow$ 30\,GB increase improves PPL by only 1.4\%. The 30\,GB model (PPL 8.657) slightly outperforms BF16 (PPL 8.728), likely due to mild regularization from 8-bit quantization.

\paragraph{Fitted prediction curve.} We fit an inverse-power model to the measured points:
\begin{equation}
    \text{PPL}(B) = 8.371 + \frac{0.494}{(B - 15.099)^{0.135}}
    \label{eq:ppl_curve}
\end{equation}
with RMSE = 0.025 PPL over the valid range $B \in [15.2, 30.0]$\,GB (95\% CI: $\pm$0.046 PPL). This enables practitioners to predict output quality for any budget target \emph{before} running the pipeline. Key thresholds: within +1\% of BF16 at 17.3\,GB, within +2\% at 15.7\,GB, BF16 parity at 26.1\,GB.

\paragraph{Scaling to 109B parameters.} Table~\ref{tab:scout_budget} demonstrates budget targeting on Llama-4-Scout, a model too large to run in BF16 on any single consumer device. From a single rate-distortion analysis pass, MINT generates optimal allocations for different hardware tiers.

\begin{table}[h]
\centering
\caption{MINT budget-targeted deployment of Llama-4-Scout (109B MoE). The same analysis produces optimal allocations for different hardware. BF16 (${\sim}$203\,GB) exceeds all single-device targets.}
...
\midrule
\multirow{2}{*}{\shortstack[l]{Qwen3-30B-A3B\\(MoE, 30B)}}
    & GPTQ Int4 (g128) & 16.0 & 9.122 & 9.160 & --- \\
    & \textbf{MINT} & \textbf{16.1} & \textbf{8.970} & \textbf{9.020} & \textbf{$-$1.5\%} \\
\midrule
\multirow{2}{*}{\shortstack[l]{Qwen2-57B-A14B\\(MoE, 57B)}}
    & GPTQ Int4 (g128) & 29.9 & 6.390 & 6.396 & --- \\
    & \textbf{MINT} & \textbf{29.9} & \textbf{6.329} & \textbf{6.356} & \textbf{$-$0.6\%} \\
\midrule
\multirow{3}{*}{\shortstack[l]{Mixtral-8x7B$^\dagger$\\(MoE, 47B)}}
    & GPTQ per-ch & 87.0$^\dagger$ & 4.608 & 4.640 & --- \\
    & Uniform 4-bit (g64) & 24.5 & 4.471 & 4.461 & --- \\
...
\midrule
\multirow{3}{*}{\shortstack[l]{Llama-4\\Scout}}
    & Uniform 4-bit & 7.899 & --- & --- \\
...
\textbf{Recommendation:} Future quantization evaluations should report both mean and median perplexity, along with outlier counts. Median PPL should be the primary comparison metric, with mean PPL treated as potentially unreliable on models with heavy-tailed per-sequence loss distributions.
...
\paragraph{Group size is the primary quality lever.}
The most striking finding is that \textbf{group-size optimization provides larger quality gains than bit-width selection}. On Qwen3-30B-A3B, the allocator chose 4-bit g32 for 85\% of tensors instead of the conventional g128. v1 kept all 4-bit tensors at g128 and instead spent its budget on 16-bit and 8-bit allocations. The knapsack determines that the quality improvement from 4$\times$ more quantization groups (g32 vs g128) at 0.125\,bytes/parameter overhead is more efficient than upgrading select tensors to 16-bit at 1.5\,bytes/parameter overhead. This validates the observation from prior work~\cite{li2025mixllm} that group size is a first-order compression variable.

\paragraph{SQNR veto catches unsafe heuristic allocations.}
On Qwen3-30B-A3B, v1's threshold heuristic allocated 4.0\% of parameters to 2-bit. The SQNR analysis reveals these allocations had SQNR values of 4.8--8.1\,dB---far below the 9\,dB safety floor. The SQNR veto provides a \emph{model-specific} safety check that catches dangerous allocations regardless of how the heuristic scoring ranks them. Our floor sweep (\S\ref{sec:sqnr_validation}) confirms that the cliff is between 2-bit and 3-bit, not between 3-bit and 4-bit.

\paragraph{16-bit allocation is targeted.}
MINT hard-protects embeddings, the LM head, layer norms, and MoE routers at 16-bit, matching the inference runtime's constraints (these modules are never quantized in MLX). v1 additionally allocated 5.6\% of \emph{quantizable} parameters to 16-bit via its heuristic thresholds. The MCKP solver never assigns 16-bit to quantizable tensors: the bytes saved by avoiding 16-bit produce more quality when spent on finer group sizes across thousands of other tensors.

\paragraph{Budget utilization enables hardware targeting.}
The greedy solver achieves 100\% budget utilization across all experiments. This is a direct advantage of the constrained formulation: a user with an iPhone (16\,GB), an RTX 4090 (24\,GB), or a Mac (48\,GB) specifies their exact available memory and MINT delivers the best achievable quality. Threshold-based methods produce a fixed model size with no user control. Combined with the PPL prediction curve (Eq.~\ref{eq:ppl_curve}), users can estimate quality \emph{before} running the pipeline, enabling informed deployment decisions. The Llama-4-Scout results demonstrate this at extreme scale: the same 109B model is deployed on a 64\,GB device (58\,GB, PPL 7.703) and a 192\,GB device (163\,GB, PPL 7.359) from a single analysis pass, with the quality difference (4.7\%) far smaller than the 2.8$\times$ size difference.

\paragraph{Scaling to models that exceed available memory.}
Llama-4-Scout (109B parameters, ${\sim}$203\,GB in BF16) cannot be loaded or evaluated in full precision on any single consumer device. This makes data-free quantization not merely convenient but \emph{necessary}---calibration-based methods would require either distributed infrastructure for calibration or reliance on proxy models. MINT's weight-only analysis processes each safetensor shard independently, never requiring the full model to reside in memory simultaneously. The analysis completes in approximately 50 minutes and produces a manifest from which any number of budget-targeted variants can be generated without re-analysis.
...
\paragraph{Quantization as regularization.}
At 30.75\,GB (97.5\% 8-bit), the knapsack model achieves PPL 8.657---\emph{below} BF16's 8.728. This 0.8\% improvement is unlikely to be pure noise (it exceeds the 0.046 PPL fit uncertainty) and suggests that moderate quantization can act as implicit regularization. This effect warrants further investigation.

\paragraph{MINT consistently outperforms calibration-based GPTQ.}
In matched-size comparisons across three MoE families (\S\ref{sec:gptq_comparison}), MINT outperforms GPTQ by $-0.6\%$ to $-4.6\%$ median PPL despite being entirely data-free. Additionally, on dense Qwen3-8B (at a larger size budget), MINT (PPL 10.039) outperforms both GPTQ (PPL 10.30) and AWQ (PPL 10.50) by 2.5\% and 4.4\% respectively. The consistency across four model families and two architecture types (dense and MoE) challenges the assumption that calibration is necessary for competitive quantization quality. We attribute this to three factors: (1) joint group-size optimization accesses a richer configuration space than fixed-group GPTQ; (2) the MCKP solver provably maximizes quality under the budget constraint rather than applying uniform bit-widths; and (3) for MoE models, calibration must adequately cover expert activation patterns across potentially hundreds of experts, which narrow calibration sets may fail to achieve.

\paragraph{Limitations.}
The allocator operates on raw NRMSE without per-tensor importance weighting; incorporating activation-aware sensitivity (e.g., Fisher information or Hessian traces) as a learned importance weight could improve allocation quality, particularly on models like GLM-4.7-Flash where the allocator disagrees with heuristic methods. The PPL prediction curve is fitted to a single model family; cross-model generalization of curve \emph{parameters} requires further validation (though the pipeline itself is model-agnostic). BF16 perplexity is unavailable for Llama-4-Scout as the model (${\sim}$203\,GB) exceeds our test system's 192\,GB memory. We also lack downstream task accuracy comparisons and throughput measurements for mixed group-size overhead. The GPTQ comparison uses publicly available GPTQ-Int4 models with default settings; comparisons against GPTQ with optimized hyperparameters (e.g., act-order with variable group sizes) could narrow the gap.
...
We presented MINT, a data-free mixed-precision quantization framework that replaces ad-hoc heuristic scoring with principled compute-optimal allocation. We evaluated MINT on four model families spanning 8B to 109B parameters, including both dense and MoE architectures. By formulating per-tensor allocation as a Multiple-Choice Knapsack Problem over joint (bit-width, group-size) configurations, MINT enables three capabilities that prior methods lack:

\begin{enumerate}
    \item \textbf{Budget-targeted deployment.} Users specify their exact memory constraint---16\,GB for iPhone, 24\,GB for RTX 4090, 64--192\,GB for Mac---and receive the provably optimal allocation. On Llama-4-Scout (109B), a single analysis pass produces deployments from 58\,GB (PPL 7.703) to 163\,GB (PPL 7.359), both outperforming uniform 4-bit. A fitted prediction curve estimates output quality before running the pipeline.
...
\midrule
\multirow{3}{*}{Qwen3-30B} & 16.3\,GB & +2.3\% & +10.3\% & 78\% \\
 & 17.4\,GB & +1.5\% & +10.3\% & 86\% \\
 & 19.0\,GB & +0.6\% & +10.3\% & 94\% \\
\midrule
...
\begin{itemize}
    ...
    \item SVD: randomized SVD with rank $k = 256$ via \texttt{torch.svd\_lowrank}

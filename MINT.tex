...
Generation throughput decreases by 2--4\% on dense models (Qwen3-30B, GLM-4.7-Flash) where MINT's overhead is minimal. The MoE model (Llama-4-Scout) shows a larger 5--18\% generation penalty and significantly slower prefill (TTFT), attributable to 8-bit override layers requiring more memory bandwidth during the compute-bound prefill phase. Prefill on dense models is largely unaffected ($<$1\% on Qwen3-30B). The throughput cost scales with the fraction of tensors assigned non-default configurations: models with fewer overrides (like GLM-4.7-Flash, where MINT achieves $-$20.8\% better PPL than uniform) incur negligible speed penalties.
...
Contrary to the conventional assumption that calibration-based methods are strictly superior, MINT consistently outperforms GPTQ at matched model sizes across three MoE families (Qwen3-30B-A3B: $-1.5\%$, Qwen2-57B-A14B: $-0.6\%$, Mixtral-8x7B: $-4.6\%$ median PPL) and also outperforms both GPTQ and AWQ on dense Qwen3-8B. This consistency across seven model families and two architecture types (dense and MoE) suggests that joint group-size optimization and budget-constrained allocation can provide quality gains that compensate for the absence of calibration data. The throughput cost of MINT's mixed-precision allocation is modest on dense models (2--4\% generation overhead, negligible prefill impact), though MoE models incur larger penalties (5--18\% generation, significant prefill slowdown) due to 8-bit override layers requiring additional memory bandwidth. For MoE models specifically, MINT's data-free approach avoids the coverage problem inherent in calibrating across hundreds of sparsely activated experts. MINT is useful both as a standalone method when calibration is impractical (very large models, proprietary data) and as a front-end for calibration-based refinement.
